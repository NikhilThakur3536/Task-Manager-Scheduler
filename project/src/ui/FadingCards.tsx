...
export const FadingCards=({Opacity,delay, icon:Icon,content,heading,progress,y,bg}:FadingCardsProps)=>{
    return(
        <div className=" absolute flex flex-col z-12 w-[80%] h-full gap-6 top-20 ">
            <motion.div className=" flex w-full h-26 bg-white rounded-2xl drop-shadow-lg items-center"
                initial={{y:50, opacity:0}}
                animate={{y:y, opacity:Opacity}}
                transition={{duration:.5, delay:delay, ease:"easeInOut"}}
            >
            <div className={`${bg} w-20 h-20 flex justify-center items-center mx-2 my-2 rounded-3xl`}><Icon size={60} color="white"/></div>
                ...
                <div className="ml-50 w-42 h-10 flex justify-center items-center border border-yellow-500/40 bg-yellow-100 rounded-2xl"><h4 className="text-yellow-700 text-2xl">{progress}</h4></div>
            </motion.div>
        </div> 
    )
